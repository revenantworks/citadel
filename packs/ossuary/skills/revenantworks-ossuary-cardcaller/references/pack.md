...
> Advisory only — consulted on boundary doubt; initial routing stays at the name + description level. **Last stamped: 2026-08-07** (two-member roster + canonical repo; generated from the registry in skillwright's `pack-registry.md`).
...
**Capstone:** none, deliberately — a two-member pack whose members already compose end to end in production every day (the cloud routine runs linecaller at 13:00 UTC; cardcaller reads what it wrote), so an orchestration prompt would only re-describe a live system. Revisit if the pack takes a third member.
