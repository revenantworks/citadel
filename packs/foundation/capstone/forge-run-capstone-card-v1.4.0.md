...
LEG 2 — lorewright · verdict mode
Niche research for {{skill_intent}}: check the registries and plugin
directories in skillwright's baseline plus the domain's own incumbents; tag
...
are applied only at need via `brandwright apply`, never stored) → evals/
authored under evalwright's doctrine when evalwright is installed
...
· LEG 0 (Run 3 only) — brandwright: verify the locally configured
...
· LEG 4 ADDENDUM (Run 3 only) — evalwright authors the suite AND issues
...
need via `brandwright apply`). No leg machinery changed; Run 2 remains the
open ladder item.
