...
**Brand-carriage law** (owner decision 2026-07-23, post-run — supersedes the 2-swap state where they differ): **the only brand carrier anywhere, repo or installs, is the locally configured brandsmith.** All other members are brandless everywhere; branded artifacts — prompt cards included — are produced at need via `brandsmith apply` and never stored. Consequence: the prompt-card swap retired; swap set 2→1 {brand-definition}; apply-install-swaps.py, RUNBOOK, and upkeep-task.md updated (script ignores a stray prompt-card.md with a retirement note). Verified same day: applied-brand sweep across all 8 skill folders clean — zero house strings, zero palette hexes, "Revenant" only as structural tokens (names, metadata.brand label, canonical-repo URL). No member files touched → no version bumps. Non-skill brand surfaces (repo `brand/` folder; structural `revenant-` naming + marketplace token) recorded as outside the skills' scope — owner's standing call.

**Phase 7 — Audit + capstone + final optimization** ✅ DONE 2026-07-23
Batches: **7A** final Fable audit (user-added) ✅ · **7B** tokensmith C-1 audit + capstone restamp ✅ · **7C** delivery + Cowork upkeep task (60d) ✅. **ALL PHASES COMPLETE.**
...
- [x] tokensmith pass — DONE 2026-07-23 as a **C-1 score-only audit** (tokensmith's own churn-restraint + already-lean doctrine governed the verdict). Measured (estimate, chars÷4 prose ratio, ±15%): bodies agentsmith 1.7k · brandsmith 2.4k · commsmith 1.8k · evalsmith 1.5k · loresmith 1.4k · promptsmith 5.8k · skillsmith 6.4k · tokensmith 2.6k ≈ **23.4k total trigger-loaded**; discovery layer (8 near-max descriptions) ≈ **2.5k tok/session always-on**. Findings: (W-dense, P2) skillsmith + promptsmith exceed the ≈5k-token gloss while well under the 500-line norm — flagged as **opportunistic-slim candidates on their next content touch** (recoverable est ~10–15% each via table/phrasing compression), NOT worth standalone churn one commit after a release: 8 version bumps + re-uploads + eval-anchor regression risk against ~1–2k tok recovered on files that load one-at-a-time. (Discovery, noted-no-action) descriptions run ~3× the ecosystem median ~80 tok/skill because they're routing-maxed near 1024 chars — the "routing wins, then trim" tradeoff, made deliberately and re-proven by the 12/12 partition. **Verdict: already lean — no rung clears the net-cost bar; pack built under tokensmith's rules from the start.**
