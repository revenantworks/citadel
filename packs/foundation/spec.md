...
- [~] U-6 normalize README skeleton ×8 — 4/8 (skillsmith, commsmith, brandsmith, agentsmith normalized to the canonical skeleton; skillsmith `configure`/brand-config/brand-inheritance refs and commsmith `voice`/voices refs removed; agentsmith stray owner's trading-spec note removed; skillsmith + commsmith SOURCES reconciled). Remaining: promptsmith, loresmith, evalsmith, tokensmith (batch 2D-2). Canonical skeleton: tagline → Workflow → Package contents → Install → Entry points → Commands & switches → Staying current → Changelog. Only Phase 2 item remaining.
