...
- OBSERVATION (not in Forge Run 3 scope): tokensmith retains a per-run `brand: <token>` report-flavor switch (C-2 neutral default, opt-in). Phase 1 decoupling was scoped to skillsmith/commsmith/brandsmith only; tokensmith's opt-in brand-output on its own HTML reports was left intact and documented as-is. If full brand centralization through brandsmith is ever wanted, that's a separate future item — flagged, not actioned.
- [x] 5F native-first Packaging rework (skillsmith) — DONE in Phase 1 (landed with the skillsmith rewrite)

**Phase 3 — New capabilities** ⬜ NEXT
