# Trigger Evals — 22 queries (12 should / 10 shouldn't)

Read each cold against name + description only. Provenance: derived from revenant-foundation-brandsmith v1.0.0, 2026-07-14.
...
| 12 | "apply the brand to the skill you're building" | SHOULD NOT — skillsmith cascade |
...
**Edge note.** Sharpest pair: 5 vs 11 — "is this on-brand" (judge against the standard) routes here; "make it on-brand" (produce the conforming message) is commsmith with an applied voice. Tuning rule: misses on 1–10 → push audit/define triggers; fires on 11–20 → tighten the two consumer boundaries in the closing sentence.
