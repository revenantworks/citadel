---
name: revenant-foundation-brandwright
...
# revenant-foundation-brandwright
...
3. **The neutral-core law.** No brand exists until one is built or handed in. Doctrine files carry zero identity content — the active definition lives only in `brand-definition.md`, and with none stored, every output defaults spec-clean neutral.
...
- `references/brand-definition.md` — **event-driven**. The active identity and voice profile; rewritten only by "brandwright build" (each build bumps the definition version and re-stamps the header), never on a clock. Ships neutral — with none defined, every output defaults spec-clean.
