...
These are craft defects — visible to a friendly reader. The Hostile read in Phase 6 hunts the other kind, a prompt that reads clean and is still cheap to satisfy badly; its catalog is `hostile-interpreter.md`.
...
**Fix:** infer everything reasonable, state the assumption, and ask only when two or more readings genuinely diverge. Always offer the just-build-it out: "just make smart assumptions."
