...
`build.py --parity` diffs the installed clone's `SKILL.md` **frontmatter** against the repo copy and exits non-zero on drift, reporting a member missing from the clone as drift too. Two honest limits: it compares frontmatter, not bodies — it catches routing drift, which is the drift that decides whether a skill fires at all — and it skips cleanly when no clone is present, which makes it CI-safe and therefore *not* a CI gate. Parity is an owner-machine step: update the clone, run parity to clean, then re-upload changed members on the surfaces where upload is manual.
