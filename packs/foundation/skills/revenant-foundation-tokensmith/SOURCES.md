...
| Cache mechanics | Prefix hits; stable-first ordering; nothing after a variable element caches; Anthropic ~1.25× write / ~0.10× read, ~5-min TTL; OpenAI auto ≥1,024 tokens ~50% | Anthropic prompt-caching docs (canonical); cross-checked via 2026 provider comparisons and practitioner write-ups | On refresh — pricing drifts |
